  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: Padding(
          ...
          child: FutureBuilder(
            future: getSbuscritions,
            builder: (BuildContext context,
                AsyncSnapshot<SubscriptionResponse> snapshot) {
              if (snapshot.connectionState == ConnectionState.done) {
                return ListView.builder(
                  itemCount: snapshot.data!.subscriptionresult.length,
                  itemBuilder: (context, index) {
                    return snapshot.data!.subscriptionresult
                            .elementAt(index)
                            .isActive
                        ? Padding(
                            padding: const EdgeInsets.all(8.0),
                              ...
                              child: Container(
                                child: Column(
                                  children: [
                                    ListTile(
                                        title: Text(
                                          snapshot.data!.subscriptionresult
                                              .elementAt(index)
                                              .name,
